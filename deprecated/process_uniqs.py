...
def write_unique_entities_to_file(table, list_uniqs, stage, header, service_dir, logger):
    logger.error(f"There are {len(list_uniqs)} unique elements in table {table} "
                 f"on {stage}-server. Detailed list of records "
                 f"saved to {service_dir}{table}_uniqRecords_{stage}")
    file_name = f"{service_dir}{table}_uniqRecords_{stage}"
